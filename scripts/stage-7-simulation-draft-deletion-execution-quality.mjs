...
check("draft-delete-owner-scoped-active-only", provider.includes('async deleteSimulationDraft(input)') && provider.includes('.eq("draft_id", input.draftId)') && provider.includes('.eq("owner_principal_id", input.ownerPrincipalId)') && provider.includes('.eq("draft_status", "active")') && provider.includes('.eq("deletion_state", "active")') && provider.includes('.is("legal_hold_reason", null)') && provider.includes('.gt("expires_at", input.deletedAt)'), "Provider mutation must target one active, non-protected, non-expired owner-scoped draft.");
...
check("draft-delete-no-other-entity-mutation", !execution.includes("deleteSimulationRecord") && !execution.includes("deleteSimulationHistory") && !execution.includes("deleteAccount") && !execution.includes("consent") && execution.includes("enforceExpiredSimulationDraftRetention") && !execution.includes("listSimulationDrafts"), "Draft deletion must not mutate saved simulations, history, accounts, or consent; only expired drafts may delegate to the existing one-draft retention transition.");
...
check("draft-delete-runtime-remains-server-boundary", !execution.includes("NextResponse") && !execution.includes("revalidatePath") && !execution.includes("redirect("), "The deletion runtime must remain independent from UI/navigation concerns.");

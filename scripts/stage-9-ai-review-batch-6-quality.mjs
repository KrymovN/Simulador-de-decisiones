...
add("pattern-registry-final", patterns.patterns.length === 9 && requiredPatterns.every((name) => patterns.patterns.some((item) => item.pattern === name && item.batch_6_assessment && Number.isInteger(item.cumulative_occurrence_count) && item.generator_or_template_linkage && item.likely_root_cause && item.recommended_remediation_scope && ["ISOLATED", "RECURRING", "POTENTIALLY_SYSTEMIC", "SYSTEMIC_BLOCKER"].includes(item.status))), "All nine final patterns contain Batch 6 and cumulative evidence.");
...
add("deterministic-regeneration", Object.entries(generatedFiles).every(([name, value]) => read("docs", "qa", "review", "ai-batches", "batch-6", name) === serialize(value)) && read("docs", "qa", "review", "AI_REVIEW_PROGRESS.json") === serialize(generated.progress) && read("docs", "qa", "review", "AI_REVIEW_CROSS_BATCH_PATTERNS.json") === serialize(generated.patterns) && read("docs", "qa", "review", "AI_REVIEW_PATTERN_SATURATION.json") === serialize(generated.saturation) && read("docs", "qa", "review", "AI_REVIEW_PRIMARY_CLOSURE.json") === serialize(generated.closure), "Regeneration is byte-identical.");

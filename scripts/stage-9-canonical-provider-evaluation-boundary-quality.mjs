...
add("evaluation-request-profile-owned-production-schema-isolated", !boundarySource.includes("buildCandidateDecisionMaterialProviderRequest") && !boundarySource.includes("OPENAI_DECISION_MATERIAL_LIMITS") && resultSource.includes("CANDIDATE_DECISION_MATERIAL_OUTPUT_SCHEMA") && resultSource.includes("levio_canonical_provider_evaluation_result_v1") && adapterSource.includes("levio_candidate_decision_material_v1"), "Evaluation request configuration must be profile-owned while the evaluation result embeds, not changes, the production candidate schema.");
...
add("evaluation-specific-output-budget", evaluation.CANONICAL_PROVIDER_EVALUATION_LIMITS.maxOutputTokens === 4000 && evaluation.CANONICAL_PROVIDER_EVALUATION_LIMITS.maxCostUsd === 0.06, "Terra evaluation-only output and conservative cost ceilings must be 4000 tokens and $0.06.");
add("evaluation-specific-generation-timeout", evaluation.CANONICAL_PROVIDER_EVALUATION_LIMITS.generationTimeoutMs === 120000 && require(join(root, "lib", "ai-provider", "openai-decision-material-adapter.ts")).OPENAI_DECISION_MATERIAL_LIMITS.generationTimeoutMs === 30000, "Evaluation generation timeout must be 120 seconds without changing the 30-second production timeout.");
add("evaluation-candidate-terra-production-terra-independent", evaluation.CANONICAL_PROVIDER_EVALUATION_CANDIDATE.model === "gpt-5.6-terra" && require(join(root, "lib", "ai-provider", "openai-decision-material-adapter.ts")).OPENAI_DECISION_MATERIAL_MODEL === "gpt-5.6-terra", "Evaluation and production may select Terra through independent configuration boundaries.");
add("evaluation-terra-pricing", evaluation.CANONICAL_PROVIDER_EVALUATION_CANDIDATE.inputUsdPerMillion === 2 && evaluation.CANONICAL_PROVIDER_EVALUATION_CANDIDATE.cachedInputUsdPerMillion === 0.2 && evaluation.CANONICAL_PROVIDER_EVALUATION_CANDIDATE.outputUsdPerMillion === 12, "Evaluation-only Terra pricing must remain $2/$0.20/$12 per million tokens.");

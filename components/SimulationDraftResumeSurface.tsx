...
export default function SimulationDraftResumeSurface({ state, saveStatus, deletionStatus }: { state: SimulationDraftResumeState; saveStatus?: string; deletionStatus?: string }) {
  ...
  return <>
    ...
    {deletionStatus && <section className="dashboard-card section-frame" role="alert"><p>{deletionStatus === "confirmation_required" ? "Confirma que comprendes que la eliminación es irreversible." : deletionStatus === "restricted" ? "Este borrador está restringido y no se puede eliminar." : "No se pudo eliminar el borrador. Su contenido permanece sin cambios."}</p></section>}
    ...
    <form action={deleteSimulationDraftFromDashboard} className="dashboard-card section-frame">
      <input name="draftId" type="hidden" value={state.draftId} />
